...
import { mockListeningExercises } from "@/data/mockMenuListening";
...
const data = mockListeningExercises;
...
export default function WritingTopicsPage() {
  ...
  const { data: clozeLessons, isLoading: clozeLoading, error: clozeError } = useLessonsByModality({
    modality: "cloze",
    enabled: slug === "cloze"
  });

  const { data: dictationLessons, isLoading: dictationLoading, error: dictationError } = useLessonsByModality({
    modality: "dictation",
    enabled: slug === "dictation"
  });
    ...
    <div className="min-h-screen bg-gray-50  px-6">
      ...
      <div className="container mx-auto px-4 py-4 relative z-10">
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <>
              {mcqLessons.map((lesson, lessonIndex) =>
                <CustomCard
                  ...
                  icon={Mail}
                  href={`/practice/listening/${exercise.slug}/${lesson.lessonId}`}
                />
              )}
            </>
          )}

          {/* Render API data for cloze */}
          {slug === "cloze" && clozeLessons && !clozeLoading && (
            <>
              {clozeLessons.map((lesson, lessonIndex) =>
                <CustomCard
                  key={`${lesson.lessonId}`}
                  slug={lesson.lessonId}
                  name={lesson.name}
          ...
          {slug === "dictation" && dictationLessons && !dictationLoading && (
            <>
              {dictationLessons.map((lesson, lessonIndex) =>
                <CustomCard
                  key={`${lesson.lessonId}`}
                  slug={lesson.lessonId}
                  name={lesson.name}
