...
const convertApiDataToGroups = (apiData: ApiTestData): Group[] => {
  ...
  const sortedParts = [...apiData.parts].sort(
  ...
  sortedParts.forEach((part: ApiPart) => {
    ...
    sortedGroups.forEach((apiGroup: ApiGroup) => {
      const questions: Question[] = apiGroup.questions.map((apiQuestion) => {
        ...
        const correctAnswerIndex = apiQuestion.answers.findIndex(
...
const getPartName = (partNumber: number): string => {
  ...
  return (
    partNames[partNumber as keyof typeof partNames] || `Part ${partNumber}`
  );
};
...
export default function ReviewTestPage({ onComplete }: ReviewTestPageProps) {
      ...
      submitReview(fullTest.id, {
        onSuccess: async (reviewResult) => {
          ...
          const transformedResult = {
            ...
            readingScore: null,
            accuracy: Math.round(
              (reviewResult.correctCount / reviewResult.totalQuestions) * 100
            ),
  ...
  const calculateResults = () => {
    ...
    const scoreMapping =
      BAND_SCORE_MAPPING[correctAnswers] || BAND_SCORE_MAPPING[0];
  ...
  const identifyWeakAreas = () => {
    ...
    const weakParts = TOEIC_PARTS.filter((part) => {
      ...
      return (
        partQuestions.length > 0 && correctCount / partQuestions.length < 0.7
      );
    });
  ...
  useEffect(() => {
    // SCRIPT 1: 80% Correct Answers (Strong Performance)
    // @ts-ignore
    window.demoFill80Percent = async (delay = 50) => {
      console.log(
        "📊 DEMO SCRIPT 1: 80% Performance - Strong student with focus areas"
      );
      ...
      // Strategically select 20% of questions to get wrong
      // Focus on Part 6 (Text Completion) and Part 7 (Inference) - weak areas
      for (let i = 0; i < questions.length; i++) {
        const q = questions[i];
        // Make Part 6 & 7 questions harder (some wrong)
        if ((q.part === 6 || q.part === 7) && Math.random() < 0.3) {
          incorrectIndices.add(i);
        }
        // Make Part 2 slightly harder
        else if (q.part === 2 && Math.random() < 0.15) {
          incorrectIndices.add(i);
        }
      }
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (q.answerIds && q.answerIds.length > 0) {
          ...
          if (incorrectIndices.has(i)) {
            // Select wrong answer for this question
            optionIndex = (q.correctAnswer + 1) % q.answerIds.length;
            console.log(`❌ Q${i + 1} (Part ${q.part}): Wrong answer selected`);
          } else {
      ...
      console.log(
        "Weak areas: Part 6 Word Choice, Part 7 Inference, Part 2 Accents"
      );
      alert(
        "✅ DEMO SCRIPT 1: Filled with 80% accuracy!\n\nProfil: Advanced student\nWeak areas: Reading inference, Part 2 unclear audio"
      );
    };

    // SCRIPT 2: 20% Correct Answers (Weak Performance)
    // @ts-ignore
    window.demoFill20Percent = async (delay = 50) => {
      console.log(
        "📊 DEMO SCRIPT 2: 20% Performance - Beginner student needs foundation"
      );
      ...
      // Only select 20% of questions to get right
      // Get Part 1 & 5 mostly correct (simpler content)
      for (let i = 0; i < questions.length; i++) {
        const q = questions[i];
        // Part 1 has highest success rate
        if (q.part === 1 && Math.random() < 0.8) {
          correctIndices.add(i);
        }
        // Part 5 some success
        else if (q.part === 5 && Math.random() < 0.3) {
          correctIndices.add(i);
        }
      }
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (q.answerIds && q.answerIds.length > 0) {
          ...
          if (correctIndices.has(i)) {
            ...
            optionIndex = Math.floor(Math.random() * q.answerIds.length);
            if (optionIndex === q.correctAnswer) {
              optionIndex = (q.correctAnswer + 1) % q.answerIds.length;
            }
            console.log(`❌ Q${i + 1} (Part ${q.part}): Wrong (needs study)`);
          }
      ...
      console.log("✅ Script 2 completed! Result: ~20% (Beginner level)");
      console.log(
        "Critical areas: Grammar, Listening comprehension, Vocabulary"
      );
      alert(
        "✅ DEMO SCRIPT 2: Filled with 20% accuracy!\n\nProfil: Beginner student\nNeeds: Foundation grammar, vocabulary, listening basics"
      );
    };

    // Original autoFill function (unchanged)
    // @ts-ignore
    window.autoFill = async (delay = 100) => {
      ...
      alert(
        "Đã điền xong tất cả đáp án và lưu lên server! Bạn có thể nộp bài."
      );
    };

    return () => {
      // @ts-ignore
      delete window.demoFill80Percent;
      // @ts-ignore
      delete window.demoFill20Percent;
      // @ts-ignore
      ...
      >
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-gray-500">
            {Math.round(Math.min(loadingProgress, 90))}% hoàn thành
          </p>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
      <div className="container mx-auto px-4 py-6">
        <AnimatePresence mode="wait">
          ...
          {(isSubmitting || isSubmittingReview) && <SubmittingScreen />}
          ...
          {stage === "results" && testResults && (
            <ResultsScreen onRetake={handleRetake} />
          )}
        </AnimatePresence>
      </div>
    </div>
...
declare global {
  interface Window {
    ...
    demoFill80Percent: (delay?: number) => Promise<void>;
    demoFill20Percent: (delay?: number) => Promise<void>;
  }
}
